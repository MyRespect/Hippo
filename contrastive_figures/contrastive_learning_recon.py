...
def split_dataset(layer_num, test_size=0.2, window_length=100):
    layer_num = 1
    file_name = './draw_figures/contrastive_data_'+str(layer_num)+'.npy'
    ...
    for key in range(len(saved_data)):
        x_dataset.extend(saved_data[key])
        y_dataset.extend(key*np.ones(len(saved_data[key])).astype(int))
...
def cnn_test(layer_num, model, loss_function, x_test, y_test, window_length=100, batch_size=32, device='cpu'):
    model.eval()
    test_loss = 0
    correct = 0
    len_test = len(x_test)
    ...
    torch.save(save_embed_label, "save_embed_label_layer"+str(layer_num)+".pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('layer_num', metavar='N', type=int, nargs='+',
    ...
    x_train, x_test, y_train, y_test = split_dataset(layer_num, test_size = 0.001, window_length=window_length)
    ...
    # torch.save(trained_model, 'contrastive_harbox_model_layer'+str(layer_num)+'.pt')

    trained_model = torch.load('contrastive_harbox_model_layer'+str(layer_num)+'.pt')
    print("Finish training and start testing: ")
    cnn_test(layer_num, trained_model, loss_function, x_train, y_train, batch_size=batch_size, device=device)
